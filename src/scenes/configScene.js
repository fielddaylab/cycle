var ConfigScene = function(game, stage)
{
  ...
  self.ready = function()
  {
    ...
    joins = [];
    jbtn_a = new ButtonBox(10,10, dc.width-20,30,function(evt){ if(hit_ui || mode != CONFIG_JOIN) return; hit_ui = true; join = joins[0]; cli.add(cli.id+" JOIN "+join); mode = CONFIG_COMMIT; });
    jbtn_b = new ButtonBox(10,50, dc.width-20,30,function(evt){ if(hit_ui || mode != CONFIG_JOIN) return; hit_ui = true; join = joins[1]; cli.add(cli.id+" JOIN "+join); mode = CONFIG_COMMIT; });
    jbtn_c = new ButtonBox(10,90, dc.width-20,30,function(evt){ if(hit_ui || mode != CONFIG_JOIN) return; hit_ui = true; join = joins[2]; cli.add(cli.id+" JOIN "+join); mode = CONFIG_COMMIT; });
    jbtn_d = new ButtonBox(10,130,dc.width-20,30,function(evt){ if(hit_ui || mode != CONFIG_JOIN) return; hit_ui = true; join = joins[3]; cli.add(cli.id+" JOIN "+join); mode = CONFIG_COMMIT; });
    jbtn_e = new ButtonBox(10,170,dc.width-20,30,function(evt){ if(hit_ui || mode != CONFIG_JOIN) return; hit_ui = true; join = joins[4]; cli.add(cli.id+" JOIN "+join); mode = CONFIG_COMMIT; });
    jbtn_f = new ButtonBox(10,210,dc.width-20,30,function(evt){ if(hit_ui || mode != CONFIG_JOIN) return; hit_ui = true; join = joins[5]; cli.add(cli.id+" JOIN "+join); mode = CONFIG_COMMIT; });
