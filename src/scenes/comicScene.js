	...
	self.draw = function () {
		...
		if (cur_img < imgs.length) {
			...
			if (cur_img - 2 >= 0) {
				...
				ctx.drawImage(
					imgs[cur_img - 2],
					lerp(slot.x, lerp_slot.x, Math.abs(delta)),
					lerp(slot.y, lerp_slot.y, Math.abs(delta)),
					lerp(slot.w, lerp_slot.w, Math.abs(delta)),
					lerp(slot.h, lerp_slot.h, Math.abs(delta))
				);
			}
			...
			ctx.beginPath();
			ctx.arc(
				lerp(node.x, lerp_node.x, Math.abs(delta)) + node.w / 2,
				node.y + node.h / 2,
				node_s / 3,
				0,
				2 * Math.PI
